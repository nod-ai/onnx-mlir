...
struct FuncTorchTypeConversionPass
  ...
  StringRef getArgument() const override { return "convert-function-types-to-torch-types"; }
...
onnx_mlir::createFuncTorchTypeConversionPass() {
...
struct FinalizingTorchTypeConversionPass
    : public PassWrapper<FinalizingTorchTypeConversionPass, OperationPass<ModuleOp>> {

  StringRef getArgument() const override { return "finalize-torch-type-conversion"; }
  ...
  void runOnOperation() override {
    ...
    setupFinalization<UnrealizedConversionCastOp>(target, patterns, typeConverter);
